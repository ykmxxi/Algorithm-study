...
class Solution {
	...
	static boolean check(ArrayList<Info> space, ArrayList<Info> piece) {
		// 90도씩 회전 시키기
		for (int i = 0; i < 4; i++) {
			int sx = piece.get(0).x;
			int sy = piece.get(0).y;
			...
			for (int j = 0; j < space.size(); j++) {
				Info spaceInfo = space.get(j);
				Info pieceInfo = piece.get(j);

				// 블록 좌표가 빈 공간의 좌표와 하나라도 다르면 중단
				if (spaceInfo.x != pieceInfo.x || spaceInfo.y != pieceInfo.y) {
					flag = false;
					break;
				}
			}
